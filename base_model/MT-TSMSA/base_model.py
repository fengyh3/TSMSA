...
class BaseModel(object):
    def __init__(self, batch_size=32, max_seq_len=100, num_attention_heads=4, size_per_head=64, num_hidden_layers=1,
                intermediate_size=512, initializer_range=0.02, emb_path=None, aspect_num_labels=None, opinion_num_labels=None):
        ...
        self.aspect_num_labels = aspect_num_labels
        self.opinion_num_labels = opinion_num_labels
        ...
        self.position_embeddings = tf.get_variable('position_embeddings_table', shape = [self.max_seq_len, 16], initializer=create_initializer(self.initializer_range))
    ...
    def set_placeholder(self):
        ...
        self.label_type = tf.placeholder(dtype = tf.int32, shape = [], name = 'label_type')
    ...
    def embedding_layer(self):
        ...
        with tf.variable_scope("word_embeddings"):
            ...
            embedding_output = tf.layers.dense(embedding_output, 112, activation=gelu, kernel_initializer=create_initializer(self.initializer_range))
    ...
    def model(self):
        ...
        crf_layer_aspect = CRF(self.num_attention_heads * self.size_per_head, create_initializer(self.initializer_range), self.aspect_num_labels, self.max_seq_len, 'aspect')
        crf_layer_opinion = CRF(self.num_attention_heads * self.size_per_head, create_initializer(self.initializer_range), self.opinion_num_labels, self.max_seq_len, 'opinion')
        
        def attention_operation(scope, inputs, attention_mask):
            input_shape = get_shape_list(inputs)
            inputs = reshape_to_matrix(inputs)
            with tf.variable_scope(scope + '_attention', reuse=tf.AUTO_REUSE):
                outputs = self.attention_layer(inputs, inputs, input_shape, num_attention_heads=1, size_per_head=self.num_attention_heads * self.size_per_head, attention_probs_dropout_prob=self.attention_dropout_placeholder,do_return_2d_tensor=True)
                outputs = tf.layers.dense(outputs, self.num_attention_heads * self.size_per_head, activation=gelu, kernel_initializer=create_initializer(self.initializer_range))
                outputs = tf.nn.dropout(outputs, 1.0 - self.hidden_dropout_placeholder)
                outputs = layer_norm(outputs + inputs)
            outputs = reshape_from_matrix(outputs, input_shape)
            return outputs

        def aspect_operation(crf, inputs, labels, lengths):
            crf.set_tensor(inputs, labels, lengths)
            rst = crf.add_crf_layer()
            return rst

        def opinion_operation(crf, inputs, labels, lengths):
            crf.set_tensor(inputs, labels, lengths)
            rst = crf.add_crf_layer()
            return rst

        standard = tf.constant(1)
        self.loss, self.logits, self.trans, self.pred_ids = tf.cond(tf.equal(self.label_type, standard), lambda: opinion_operation(crf_layer_opinion, final_output, self.labels,
                                                            lengths), lambda: aspect_operation(crf_layer_aspect, final_output, self.labels, lengths))

    def train(self, x_aspect, y_aspect, x_opinion, y_opinion, dev_x_aspect, dev_y_aspect, dev_x_opinion, dev_y_opinion, epochs, hidden_dropout=0.1, attention_dropout=0.1, learning_rate=0.02, aspect_label_list=None, opinion_label_list=None):
        ...
        for epoch in range(epochs):
            batches_x_aspect, batches_y_aspect = get_batches(self.batch_size, x_aspect, y_aspect, is_random=True)
            batches_x_opinion, batches_y_opinion = get_batches(self.batch_size, x_opinion, y_opinion, is_random=True)
            loss_sum = 0.0
            for i in range(max(len(batches_x_aspect), len(batches_x_opinion))):
                if i < len(batches_x_aspect):
                    feed_dict = {self.input_ids:batches_x_aspect[i], self.labels:batches_y_aspect[i], self.attention_dropout_placeholder:attention_dropout, self.hidden_dropout_placeholder:hidden_dropout, self.label_type:0}
                    _, loss, pred_labels = self.sess.run([self.optimizer, self.loss, self.pred_ids], feed_dict = feed_dict)
                    loss_sum += loss
                if i < len(batches_x_opinion):
                    feed_dict = {self.input_ids:batches_x_opinion[i], self.labels:batches_y_opinion[i], self.attention_dropout_placeholder:attention_dropout, self.hidden_dropout_placeholder:hidden_dropout, self.label_type:1}
                    _, loss, pred_labels = self.sess.run([self.optimizer, self.loss, self.pred_ids], feed_dict = feed_dict)
                    loss_sum += loss
            print('epoch %d loss: %s' % (epoch, loss_sum))
            predict_ids = self.predict(dev_x_aspect, 0)
            predict_ids.extend(self.predict(dev_x_opinion, 1))
            label_types = np.zeros(shape=[len(dev_x_aspect)])
            label_types = np.concatenate([label_types, np.ones(shape=[len(dev_x_opinion)])])
            aspect_id2label = {}
            opinion_id2label = {}
            for idx, label in enumerate(aspect_label_list):
                aspect_id2label[idx + 1] = label
            for idx, label in enumerate(opinion_label_list):
                opinion_id2label[idx + 1] = label

            labels = []
            for idx, label in enumerate(dev_y_aspect + dev_y_opinion):
                lab = []
                for l in label:
                    if int(l) == 0:
                        break
                    if label_types[idx] == 0:
                        lab.append(aspect_id2label[int(l)])
                    else:
                        lab.append(opinion_id2label[int(l)])
                labels.append(lab)
            ...
            for idx, predict in enumerate(predict_ids):
                ...
                for i in range(len(labels[idx])):
                    if predict[i] != 0:
                        if label_types[idx] == 0:
                            pred.append(aspect_id2label[predict[i]])
                        else:
                            pred.append(opinion_id2label[predict[i]])
                    else:
                        pred.append('O')
                predict_labels.append(pred)

            evaluate(predict_labels, labels, label_types)


    def predict(self, x, label_type, y=None):
        batches_x = get_predict_batches(self.batch_size, x)
        results = []
        for batch_x in batches_x:
            feed_dict = {self.input_ids:batch_x, self.attention_dropout_placeholder:0.0, self.hidden_dropout_placeholder:0.0, self.label_type:label_type}
